...
| No LISTEN/NOTIFY | Disable the `LISTEN/NOTIFY` listener and per-queue notifications. | Workers rely on polling only; `useListenNotify` and the queue `notify` option have no effect. | `noListenNotify` |

Lock-free fetch, split-statement writes, and disabling LISTEN/NOTIFY are **runtime** behaviors; the
other four are **schema** choices applied at install/migration time. CockroachDB needs all seven;
other distributed engines need only a subset (see below). One further CockroachDB adjustment —
coercing text-encoded integers back to numbers — is keyed on `backend === 'cockroachdb'` directly
(see below).
...
(`noCoveringIndexes`), no LISTEN/NOTIFY (`noListenNotify`), and numeric coercion on read — see
